...
export class ScoreService {
  ...
  public async scoreApi(url: string, startPoint: number = 10) {
    ...
    } = await this.getPointToReduceAndTargetUserIdIfExistsOfGetAllMembersApi(
      url,
    );

    const { targetMemberPointToReduce, targetMemberReductionReasons } =
      await this.getPointToReduceOfTargetMemberApi(url, targetUserId);

    const {
      exceptionHandlingPointToReduce,
      exceptionHandlingReductionReasons,
    } = await this.getPointToReduceOfExceptionHandling(url);
  ...
  private async getPointToReduceAndTargetUserIdIfExistsOfGetAllMembersApi(
    url: string,
    tried = 0,
  ): Promise<{
  ...
  private async getPointToReduceOfTargetMemberApi(
    url: string,
  ...
  private async getPointToReduceOfExceptionHandling(url: string) {
